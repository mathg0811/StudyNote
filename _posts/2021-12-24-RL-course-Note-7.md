...
$$ \mathsf{ J_{av} v(\theta) = \displaystyle\sum_s d^{\pi_\theta} (s) V^{\pi_\theta} (s) } $$

- Or the average reward per time-step

$$ \mathsf{ J_{av} v(\theta) = \displaystyle\sum_s d^{\pi_\theta} (s) \displaystyle\sum_a \pi_\theta (s,a) \mathcal{R}^a_s } $$
...
- Replaces instantaneous reward r with long-term value $\mathsf{Q^\pi(s,a)}$
